...
Milter.prototype.setmlreply = function() {
	...
	for( i = 0; i < args; i++) {
		...
		data = Buffer.concat([data, new Buffer(xcode), new Buffer(i === (args-1) ? ' ' : '-')]);
		data = Buffer.concat([data, new Buffer(rcode), new Buffer(' ')]);
...
Milter.prototype.header = function(ctx, cmd, index, field, value, callback) {
...
Milter.prototype.addheader = function(ctx, field, value, callback) {
	...
	this.header(ctx, SMFIR.ADDHEADER, -1, field, value, callback);
};
...
Milter.prototype.chgheader = function(ctx, field, index, value, callback) {
	...
	this.header(ctx, SMFIR.CHGHEADER, index, field, value, callback);
};
...
Milter.prototype.addrcpt = function(ctx, rcpt, callback) {
	...
	ctx.write_commmand(SMFIR.ADDRCPT, rcpt, callback);
};
...
Milter.prototype.delrcpt = function(ctx, rcpt, callback) {
	...
	ctx.write_commmand(SMFIR.DELRCPT, rcpt, callback);
};
