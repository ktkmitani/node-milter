...
function write_command_stream(ctx, cmd, stream, callback) {
	...
	function WriteCommandStream(opts) {
		opts = opts || {};
		opts.decodeString = true;
		...
		this.on('error', function(err) {
			self._error = MI_FAILURE;
		});
	}
	util.inherits(WriteCommandStream, Writable);

	WriteCommandStream.prototype._write = function(chunk, encoding, callback) {
		var self = this;
		...
		write_command(ctx, cmd, chunk, callback);
	};

	var writeCommandStream = new WriteCommandStream();
	stream.pipe(writeCommandStream);
