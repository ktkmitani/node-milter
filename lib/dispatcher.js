...
var cmds = [
	{ cmd: SMFIC.ABORT,   next: ST.ABRT, todo: CT_CONT, macro: CI_NONE, func: 'abort' },
	{ cmd: SMFIC.MACRO,   next: ST.NONE, todo: CT_KEEP, macro: CI_NONE, func: 'macros' },
	{ cmd: SMFIC.BODY,    next: ST.BODY, todo: CT_CONT, macro: CI_NONE, func: 'bodychunk' },
	{ cmd: SMFIC.CONNECT, next: ST.CONN, todo: CT_CONT, macro: CI_CONN, func: 'connectinfo' },
	{ cmd: SMFIC.BODYEOB, next: ST.ENDM, todo: CT_CONT, macro: CI_EOM,  func: 'bodyend' },
	{ cmd: SMFIC.HELO,    next: ST.HELO, todo: CT_CONT, macro: CI_HELO, func: 'helo' },
	{ cmd: SMFIC.HEADER,  next: ST.HDRS, todo: CT_CONT, macro: CI_NONE, func: 'header' },
	{ cmd: SMFIC.MAIL,    next: ST.MAIL, todo: CT_CONT, macro: CI_MAIL, func: 'sender' },
	{ cmd: SMFIC.OPTNEG,  next: ST.OPTS, todo: CT_CONT, macro: CI_NONE, func: 'optionneg' },
	{ cmd: SMFIC.EOH,     next: ST.EOHS, todo: CT_CONT, macro: CI_EOH,  func: 'eoh' },
	{ cmd: SMFIC.QUIT,    next: ST.QUIT, todo: CT_END,  macro: CI_NONE, func: 'quit' },
	{ cmd: SMFIC.DATA,    next: ST.DATA, todo: CT_CONT, macro: CI_DATA, func: 'data' },
	{ cmd: SMFIC.RCPT,    next: ST.RCPT, todo: CT_IGNO, macro: CI_RCPT, func: 'rcpt' },
	{ cmd: SMFIC.UNKNOWN, next: ST.UNKN, todo: CT_IGNO, macro: CI_NONE, func: 'unknown' },
	{ cmd: SMFIC.QUIT_NC, next: ST.Q_NC, todo: CT_CONT, macro: CI_NONE, func: 'quit' },
];
...
Dispatcher.prototype.dispatch = function(cmd, data, callback) {
	...
	var call_abort = ST_IN_MAIL(self.curstate);
	callback = callback || function(){};
	...
	async.waterfall([
		...
		function(callback) {
			...
			call_abort = ST_IN_MAIL(self.curstate);
			...
			self.sendreply(status, function(result) {
...
Dispatcher.prototype.abort = function(data, macro, callback) {
...
Dispatcher.prototype.macros = function(data, macro, callback) {
	...
	ctx.mac_buf[index] = argv;
	callback(SMFIS._KEEP);
};

Dispatcher.prototype.bodychunk = function(data, macro, callback) {
...
Dispatcher.prototype.connectinfo = function(data, macro, callback) {
	debuglog('connectinfo called');

	var ctx = this.ctx;
	var fi_connect = ctx.milter.connect;

	ctx.clear_macros(macro+1);
...
Dispatcher.prototype.bodyend = function(data, macro, callback) {
...
Dispatcher.prototype.helo = function(data, macro, callback) {
...
Dispatcher.prototype.header = function(data, macro, callback) {
...
Dispatcher.prototype.sender = function(data, macro, callback) {
...
Dispatcher.prototype.optionneg = function(data, macro, callback) {
...
Dispatcher.prototype.eoh = function(data, macro, callback) {
	debuglog('eoh called');
...
Dispatcher.prototype.quit = function(data, macro, callback) {
...
Dispatcher.prototype.data = function(data, macro, callback) {
...
Dispatcher.prototype.rcpt = function(data, macro, callback) {
...
Dispatcher.prototype.unknown = function(data, macro, callback) {
...
Dispatcher.prototype.sendreply = function(status, callback) {
	...
	switch(status) {
	case SMFIS.CONTINUE:
		ctx.write_command(SMFIR.CONTINUE, callback);
		return;
	case SMFIS.TEMPFAIL:
	case SMFIS.REJECT:
		if (ctx.reply &&
			((status === SMFIS.TEMPFAIL && ctx.reply[0] === 0x34) ||
			 (status === SMFIS.REJECT && ctx.reply[0] === 0x35))) {
			ctx.write_command(SMFIR.REPLYCODE, ctx.reply, function(result) {
				ctx.replay = null;
				callback(result);
			});
		} else {
	...
	case SMFIS.DISCARD:
		...
		ctx.write_command(SMFIR.DISCARD, callback);
		return;
	case SMFIS.ACCEPT:
		ctx.write_command(SMFIR.ACCEPT, callback);
		return;
	case SMFIS.SKIP:
		ctx.write_command(SMFIR.SKIP, callback);
		return;
	case SMFIS._OPTIONS:
		...
		for(var i = 0; i < MAX_MACROS_ENTRIES; i++) {
			...
			buf.writeUInt32BE(i);
			data = Buffer.concat([data, buf, new Buffer(ctx.mac_list[i])]);
		}

		ctx.write_command(SMFIC.OPTNEG, data, callback);
		return;
	case SMFIS.NOREPLY:
		if (bit !== 0 &&
			(ctx.pflags & bit) !== 0 &&
			(ctx.mta_pflags & bit) === 0) {
			ctx.write_command(ctx.socket, SMFIR.CONTINUE, callback);
			return;
		}
		break;
	}
