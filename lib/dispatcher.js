...
Dispatcher.prototype.dispatch = function(cmd, data, callback) {
	...
	async.waterfall([
		...
		function(callback) {
			...
			Dispatcher.prototype[cmds[index].func].call(self, data, macro, function(status) {
...
Dispatcher.prototype.sendreply = function(status, ctx, callback) {
	...
	case SMFIS.REJECT:
		if (ctx.replay &&
			...
			ctx.write_command(SMFIR.REPLYCODE, ctx.replay, function(result) {
				ctx.replay = null;
				callback(result);
			});
		} else {
