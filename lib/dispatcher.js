...
Dispatcher.prototype._execute = function(data, callback) {
	...
	if (data.length > MILTER_LEN_BYTES) {
		...
		if ((len - 1) > Maxdatasize) {
			debuglog('[%s] too big data size (len = %d, max = %d)', ctx._id, (len - 1), Maxdatasize);
			ctx._socketend();
			callback();
			...
			self._buffer = data.slice(MILTER_LEN_BYTES + len);
			data = new Buffer(0);
...
Dispatcher.prototype._dispatch = function(cmd, data, callback) {
	...
	debuglog('[%s] got cmd \'%s\' len %d', ctx._id, cmd, data.length);
	...
	async.waterfall([
		function(callback) {
			...
			if (index < 0) {
				debuglog('[%s] cmd \'%s\' unknown', ctx._id, cmd);
				callback(MI_FAILURE);
				return;
			}
			callback();
		...
		function(callback) {
			...
			debuglog('[%s] cur %d new %d nextmask 0x%s',
				ctx._id,
			...
			if (newstate !== ST.NONE && !trans_ok(self._curstate, newstate)) {
				...
				debuglog('[%s] abort: cur %d (0x%s) new %d (0x%s) next 0x%s',
					ctx._id,
		...
		function(newstate, callback) {
			...
			self[cmds[index].func](data, macro, function(status) {
				debuglog('[%s] status = %d', ctx._id, status);
...
Dispatcher.prototype._abort = function(data, macro, callback) {
	debuglog('[%s] abort called', this._ctx._id);
...
Dispatcher.prototype._macros = function(data, macro, callback) {
	debuglog('[%s] macros called', this._ctx._id);
...
Dispatcher.prototype._bodychunk = function(data, macro, callback) {
	debuglog('[%s] bodychunk called', this._ctx._id);
...
Dispatcher.prototype._connectinfo = function(data, macro, callback) {
	debuglog('[%s] connectinfo called', this._ctx._id);
	...
	if (family !== SMFIA_UNKNOWN) {
		if (i + 2 >= data.length) {
			debuglog('[%s] connect: wrong len %d >= %d', ctx._id, i, data.length);
			callback(SMFIS._ABORT);
			return;
		}
...
Dispatcher.prototype._bodyend = function(data, macro, callback) {
	debuglog('[%s] bodyend called', this._ctx._id);
...
Dispatcher.prototype._helo = function(data, macro, callback) {
	debuglog('[%s] helo called', this._ctx._id);
...
Dispatcher.prototype._header = function(data, macro, callback) {
	debuglog('[%s] header called', this._ctx._id);
...
Dispatcher.prototype._sender = function(data, macro, callback) {
	debuglog('[%s] sender called', this._ctx._id);
...
Dispatcher.prototype._optionneg = function(data, macro, callback) {
	debuglog('[%s] optionneg called', this._ctx._id);
	...
	if (data.length < MILTER_OPTLEN) {
		debuglog('[%s] %s: optionneg: len too short %d < %d',
			ctx._id, ctx._milter._name, data.length, MILTER_OPTLEN);
		callback(SMFIS._ABORT);
		return;
	}
	...
	if (v < SMFI_PROT_VERSION_MIN) {
		debuglog('[%s] %s: optionneg: protocol version too old %d < %d',
			ctx._id, ctx._milter._name, v, SMFI_PROT_VERSION_MIN);
		callback(SMFIS._ABORT);
		return;
	}
		...
		fi_negotiate(ctx,
			...
			function(status, aflags, pflags, f2, f3) {
				if (status === SMFIS.ALL_OPTS) {
					...
					debuglog('[%s] %s: negotiate returned %d (protocol options=0x%s, SMFIR=0x%s)',
						ctx._id,
	...
	function optionneg(callback) {
		...
		debuglog('[%s] milter_negotiate: mta_actions=0x%s, mta_flags=0x%s actions=0x%s, flags=0x%s',
			ctx._id,
...
Dispatcher.prototype._eoh = function(data, macro, callback) {
	debuglog('[%s] eoh called', this._ctx._id);
...
Dispatcher.prototype._quit = function(data, macro, callback) {
	debuglog('[%s] quit called', this._ctx._id);
...
Dispatcher.prototype._data = function(data, macro, callback) {
	debuglog('[%s] data called', this._ctx._id);
...
Dispatcher.prototype._rcpt = function(data, macro, callback) {
	debuglog('[%s] rcpt called', this._ctx._id);
...
Dispatcher.prototype._unknown = function(data, macro, callback) {
	debuglog('[%s] unknown called', this._ctx._id);
...
Dispatcher.prototype._sendreply = function(status, callback) {
	debuglog('[%s] sendreply called', this._ctx._id);
	...
	if (bit !== 0 && (ctx._pflags & bit) !== 0 && status !== SMFIS.NOREPLY) {
		if (status >= SMFIS.CONTINUE && status < SMFIS._KEEP) {
			debuglog('[%s] %s: milter claimed not to reply in state %d but did anyway %d',
				ctx._id, ctx._milter._name, ctx._state, status);
		}
