...
function Context(milter, socket) {
	...
	socket.on('end', function() {
		debuglog('[%s] socket end', self._id);
	});

	socket.on('error', function(error) {
		debuglog('[%s] socket error [%s]', self._id, error.message);
	});
}
...
Context.prototype._write_command = function(cmd, data, callback) {
	debuglog('[%s] write_command called', this._id);
	...
	debuglog('[%s] cmd = \'%s\' data length = %d', this._id, cmd, data.length);
